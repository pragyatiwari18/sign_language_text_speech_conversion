...
# Voice selector block goes here (add this block)
        self.voice_var = tk.StringVar(self.root)
        self.voice_var.set('Male')  # Default to Male
        voice_frame = tk.Frame(control_frame, bg='#2C3E50')
        voice_frame.pack(side=tk.LEFT, padx=20)
        voice_label = tk.Label(voice_frame, text="Voice:", font=("Arial", 12), bg='#2C3E50', fg='white')
        voice_label.pack(side=tk.LEFT)
        voice_menu = tk.OptionMenu(voice_frame, self.voice_var, 'Male', 'Female')
    ...
    def speak_fun(self):
     threading.Thread(
        ...
        args=(self.str, self.voice_var.get()), 
        daemon=True).start()

    def _speak_worker(self, to_say, gender):
     try:
        engine = pyttsx3.init()
        engine.setProperty("rate", 100)
        voices = engine.getProperty("voices")

        # Attempt to select male or female voice by gender
        if gender == "Male":
            for v in voices:
