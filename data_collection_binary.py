...
while True:
    try:
        ...
        if hands:
            ...
            #starting height calc kri
            #fir width calculate kri
            w = test_image1.shape[1]
            img_final1[((400 - h) // 2):((400 - h) // 2) + h, ((400 - w) // 2):((400 - w) // 2) + w] = test_image1
# fir yha pr image final m 400 joh grayimage ka matrix size hai usme fit kri
        ...
        if hands:
            ...
            hand = hands[0]#detect krra hai phle hath ko
            x, y, w, h = hand['bbox']#extract krega bounding box co-ordinates
            image = frame[y - offset:y + h + offset, x - offset:x + w + offset]#isse ek frame/ya regoin of interest create hoga with some extra offset space
            white = cv2.imread("white.jpg")
          
            handz = hd2.findHands(image, draw=False, flipType=True)#roi ke undar hath dhundhna,draw se no drawing
            if handz:#Agar hath mila tb..
                hand = handz[0]
                pts = hand['lmList']#line bnaega greencolor ki(0,255,0),res point(0,0,255)
            ...
            gray1 = cv2.cvtColor(roi1, cv2.COLOR_BGR2GRAY)#grayscale conversion
            blur1 = cv2.GaussianBlur(gray1, (1, 1), 2)#blurred
        ...
        print("==",traceback.format_exc() )#error handling

capture.release()#capturing vd from camera
cv2.destroyAllWindows()#close krega window
